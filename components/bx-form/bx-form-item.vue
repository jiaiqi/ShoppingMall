  ...
  <view>
    <view class="cu-form-group " v-if="fieldData.display">
      ...
      <view class="" v-if="pageFormType === 'form' || pageFormType === 'add' || pageFormType === 'update'" >
        ...
        ></textarea>
        <view class="content padding-0" style="padding:0;width: 100%!important;flex-direction: column;position: relative;" v-else-if="fieldData.type === 'date' || fieldData.type === 'dateTime' || fieldData.type === 'time'">
          <input
            @blur="onInputBlur"
            @input="onInputChange"
            v-model="fieldData.value"
            ...
            name="input"
          />
          <text class="input-icon cuIcon-calendar text-gray" style="position: absolute;top:20upx;right: 20upx;" @click.stop="fieldData.disabled ? false : toggleTab(fieldData.type)"></text>
			...
			<view class="cu-list menu card-menu ">
				<view class="cu-item" v-for="(item, index) in optionsDatasRun" :key="index">
					<view class="content solid">
						<text class="cuIcon-tagfill text-red lines-gray margin-right-xs">{{index+1 }}</text>
						<input class="input-sm" type="text" placeholder="请输入" v-model="item['model'][fieldData.optionsConfig['key_col']['value']]" />
					</view>
					<view class="action">
						<view v-if="item.type === 'draft'" class="cu-btn  bg-green light"  style="height:2.4em;min-height: 1.6em;line-height: 1.6em;" @click="addListOptions(index)">保存</view>
						<view v-if="item.type !== 'draft'" class="cu-btn  bg-orange light" style="height:2.4em;min-height: 1.6em;line-height: 1.6em;" @click="deleteListOptions(index)">删除</view>
		...
		<block  v-if="fieldData.buttons && fieldData.buttons.length > 0">
			<view class="grid text-center col-4">
				<view class="padding-sm line-blue" 
				@click.stop="onButtons(fieldData, btn)"
				:key="index"
			    v-for="(btn, index) in fieldData.buttons">
  ...
  methods: {
	  addListOptions(index){
		 ...
		 // this.optionsDatasRun[index]
		  ...
		  // for(let key in this.fieldData.optionsConfig.model){
			 //  datas[this.fieldData.optionsConfig.model[key]] = this.optionsDatasRun[index][this.fieldData.optionsConfig.model[key]]
		  // }
		  let req = [
				{
					"serviceName":sers,
			...
			if(self.optionsDatasRun[index]["type"] === "draft"){
			...
			}else if(self.optionsDatasRun[index]["type"] === "update"){
