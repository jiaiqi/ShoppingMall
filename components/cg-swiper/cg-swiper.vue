<template>
	<view class="" style="position: relative;">
		...
		>
			<swiper-item style="width: 100%;">
				<view class="swiper-item">
					<view class="swiper-item-view" v-for="(item, index) in gridData" :key="index" v-if="index < 10" @click="clickItem(item)">
						<view  class="crip" :style="{background:bgcolor[index]}" >{{item.title[0]}}</view>
						<text>{{ item.title }}</text>
					</view>
				</view>
			</swiper-item>
			
			<block v-if="gridData.length > 10">
				<swiper-item style="width: 100%;">
					<view class="swiper-item">
						<view class="swiper-item-view" v-for="(item, index) in gridData" :key="index" v-if="index > 9 && index < 20" @click="clickItem(item)">
						<view  class="crip" :style="{background:bgcolor[index]}" >{{item.title[0]}}</view>
							<text>{{ item.title }}</text>
						</view>
					</view>
		...
		<view class="dots" v-if="customize == true && swiperDots == false">
			<block v-for="(item, index) in gridData" :key="index" v-if="index < 2 && gridData.length > 10">
				<view
					class="dot"
...
export default {
	...
	created() {
		...
		}else if(this.gridData.length<=5){
