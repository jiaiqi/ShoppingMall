  ...
  methods: {
    ...
    async getWxUserInfo() {
      ...
      if (res.data.state === 'SUCCESS') {
        ...
        this.$store.commit('setWxUserInfo', wxUser);
      ...
      if (isWeixinClient) {
        ...
        that.$http.post(url, req).then(response => {
          if (response.data.resultCode === 'SUCCESS' && response.data.response[0].response) {
            ...
            // 根据&api.isRegMember 逻辑处理，调用会员加入接口。
            if (that.$api.isRegMember.isReg) {
              let user_no = that.$store.getters.getLoginUserInfo ? that.$store.getters.getLoginUserInfo.user_no : uni.getStorageSync('login_user_info').user_no;
              let up_user_no = that.$store.getters.getUpUser ? that.$store.getters.getUpUser : uni.getStorageSync('up_user') ? uni.getStorageSync('up_user') : '自主进入';
              if (up_user_no === '_shopmall') {
                up_user_no = '商家邀请';
              }
              let wxUserInfo = {};
              that.getWxUserInfo().then(data => {
                if (this.$store.getters.getLoginUserInfo.nickname) {
                  wxUserInfo = this.$store.getters.getLoginUserInfo;
                  console.log('wxUserInfo1', wxUserInfo);
                } else if (this.$store.getters.getWxUserInfo.nickname) {
                  wxUserInfo = this.$store.getters.getWxUserInfo;
                  console.log('wxUserInfo2', wxUserInfo);
                } else if (uni.getStorageSync('wxUserInfo') && uni.getStorageSync('wxUserInfo').nickname) {
                  wxUserInfo = uni.getStorageSync('wxUserInfo');
                  console.log('wxUserInfo3', wxUserInfo);
                } else {
                  wxUserInfo = this.$store.getters.getLoginUserInfo;
                }
                let e = {
      ...
      if (that.isInvalid(that.user.user_no) && that.isInvalid(this.user.pwd)) {
        ...
        if (response.data.state === 'SUCCESS' && response.data.response[0].response) {
          ...
          if (res.login_user_info.user_no) {
            ...
            that.$store.commit('setLoginUserInfo', res.login_user_info);
          ...
          if (that.$api.isRegMember.isReg) {
            ...
            if (this.$store.getters.getLoginUserInfo.nickname) {
              ...
              wxUserInfo = this.$store.getters.getWxUserInfo.nickname;
              console.log('wxUserInfo3', wxUserInfo);
            }
