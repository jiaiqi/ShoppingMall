export default {
  install(Vue, options) {
    ...
    Vue.prototype.getCoulmnConfig = function(e) {
      ...
      switch (e.item_type) {
        case "文本":
          ...
          break;
        case "选项":
          cnCol.item_type_attr['radioType'] = e.option_type === '单选' ? 'single' : e.option_type === '多选' ? 'multi' :
            ''
          break;
        case "时间日期":
    ...
    Vue.prototype.bxDataFilter = function(data, cond) {
        let datas = data
        let conds = cond
        datas = datas.filter((item) => {
          ...
          for (let i = 0; i < conds.length; i++) {
            switch (conds[i]["ruleType"]) {
              case "eq":
                if (item[conds[i]["colName"]] === item[conds[i]["value"]]) {
                  isNum++
                  resItem = item
                }
                break;
              case "neq":
                ...
                break;
              case "in":
                if (item[conds[i]["colName"]].indexOf(item[conds[i]["value"]]) !== -1) {
                  isNum++
                  resItem = item
                }
                break;
              case "isnull":
                if (item[conds[i]["colName"]] === null) {
                  isNum++
                  resItem = item
                }
                break;
      ...
      /**
       * 获取url参数 有name时返回name对应字段的值 否则返回所有参数
       * @param {Object} name 要获取的参数
       */
      Vue.prototype.getUrlParams = function(name) {
        let str = window.location.href;
        str = str.split('?')[1];
        str = Vue.prototype.getDecodeUrl(str)
        if(str){
          let arr = str.split('&');
          let obj = {};
          arr.forEach(item => {
